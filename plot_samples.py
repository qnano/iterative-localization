...
def readdata(setnumber, subsets):
    ...
    saveJ = 'J-iter-' + str(int(itermax)) + '-thetaI-' + str(int(thetaI)) + '-m-' + str(int(100*m))+ '-alpha-' + str(int(100*alpha)) + '-thetab-' + str(int(thetab))+ '.npy'
    ...
    savesmp ='smp-iter-' + str(itermax) + '-thetaI-' + str(int(thetaI)) + '-m-' + str(int(100*m)) + '-alpha-' + str(int(100*alpha)) + '-thetab-' + str(int(thetab)) +'.npy'
...
def plot(plotnumber):
    '''
    Reading data, processing and plotting of Figures 2a, 2b, 2c, S4
    ...
    if plotnumber=="2a":
        Jinv, thetaMAP, smp  = readdata(105, subsets)
        maxval = np.max(smp[0])
    ...
    if plotnumber=="2c":
        ...
        for k in range(3):
            ...
            axes[k].plot(xg,yg)
            axes[k].text(-10, 1.09*max(counts), '${\sigma}_x$ = %.1f nm' % p_opt.x[1])
    ...
    if plotnumber=="S4":
        Jinv, thetaMAP, smp  = readdata(36, subsets)
        alpha = 3
        theta = vti.estimands();
        roisize,m,kx,ky,sigmap,other = vti.imgparams();
        vmax = np.max(smp[0])
        xmax = roisize
        xrange = np.linspace(0,xmax,101)
        yrange = np.linspace(0,xmax,101)
        
        for k in range(len(Jinv)):
            if k == 0:
                phi_plusx =   kx*theta[0,0] - np.pi/2;     
                phi_minusx=   kx*theta[0,0] - np.pi/2;    
                phi_plusy=   ky*theta[0,1] - np.pi/2; 
                phi_minusy=   ky*theta[0,1] - np.pi/2;
            
            else:
                phi_plusx  =   kx*thetaMAP[0,k-1,0] + np.pi/2 +   alpha * kx*(Jinv[k-1,0,0])**0.5; 
                phi_minusx =   kx*thetaMAP[0,k-1,0] + np.pi/2 -   alpha * kx*(Jinv[k-1,0,0])**0.5;
                phi_plusy  =   ky*thetaMAP[0,k-1,1] + np.pi/2 +   alpha * kx*(Jinv[k-1,1,1])**0.5; 
                phi_minusy =   ky*thetaMAP[0,k-1,1] + np.pi/2 -   alpha * kx*(Jinv[k-1,1,1])**0.5;
        
            
            I_plusx    =   vti.intensity(theta[0,0],phi_plusx);
            I_minusx   =   vti.intensity(theta[0,0],phi_minusx);
            I_plusy    =   vti.intensity(theta[0,1],phi_plusy);
            I_minusy   =   vti.intensity(theta[0,1],phi_minusy);
            I_sum      =   I_plusx + I_minusx + I_plusy + I_minusy;
        
            fig, ax = plt.subplots(ncols = 4, nrows = 3)
            fig.suptitle('Iteration ' + str(k+1) +' of 3', fontsize=12)
            
            for pat in range(4):
                ax[0,pat].imshow(smp[0,4*k+pat], cmap = 'inferno', aspect="auto", vmin=0, vmax=vmax)
                ax[0,pat].get_xaxis().set_ticks([])
                ax[0,pat].get_yaxis().set_ticks([])
            
            z = [[vti.intensity(x,phi_plusx) for y in yrange] for x in xrange]
            ax[1,0].imshow(z, cmap='inferno', aspect="auto")
            ax[1,0].plot(51, 51, marker='*', mec='k', mfc='w', ms = 12)
            ax[1,0].get_xaxis().set_ticks([])
            ax[1,0].get_yaxis().set_ticks([])
            
            z = [[vti.intensity(x,phi_minusx) for y in yrange] for x in xrange]       
            ax[1,1].imshow(z, cmap='inferno', aspect="auto")
            ax[1,1].plot(51, 51, marker='*', mec='k', mfc='w', ms = 12)
            ax[1,1].get_xaxis().set_ticks([])
            ax[1,1].get_yaxis().set_ticks([])
            
            z = [[vti.intensity(y,phi_plusy) for y in yrange] for x in xrange]       
            ax[1,2].imshow(z, cmap='inferno', aspect="auto")
            ax[1,2].plot(51, 51, marker='*', mec='k', mfc='w', ms = 12)
            ax[1,2].get_xaxis().set_ticks([])
            ax[1,2].get_yaxis().set_ticks([])
            
            z = [[vti.intensity(y,phi_minusy) for y in yrange] for x in xrange]     
            ax[1,3].imshow(z, cmap='inferno', aspect="auto")
            ax[1,3].plot(51, 51, marker='*', mec='k', mfc='w', ms = 12)         
            ax[1,3].get_xaxis().set_ticks([])
            ax[1,3].get_yaxis().set_ticks([])
            
            ax[2,0].plot(xrange, theta[0,2]/I_sum*vti.intensity(xrange, phi_plusx), color = 'k')
            ax[2,0].plot(theta[0,0], theta[0,2]/I_sum*vti.intensity(theta[0,0], phi_plusx), marker='*', mec='k', mfc='w', ms = 12)
            ax[2,0].get_xaxis().set_ticks([])
            ax[2,0].set_ylabel('Signal photons')
            
            ax[2,1].plot(xrange, theta[0,2]/I_sum*vti.intensity(xrange, phi_minusx), color = 'k')
            ax[2,1].plot(theta[0,0], theta[0,2]/I_sum*vti.intensity(theta[0,0], phi_minusx), marker='*', mec='k', mfc='w', ms = 12)
            ax[2,1].get_xaxis().set_ticks([])
            ax[2,1].get_yaxis().set_ticks([])
              
            ax[2,2].plot(xrange, theta[0,2]/I_sum*vti.intensity(xrange, phi_plusy), color = 'k')
            ax[2,2].plot(theta[0,1], theta[0,2]/I_sum*vti.intensity(theta[0,1], phi_plusy), marker='*', mec='k', mfc='w', ms = 12)
            ax[2,2].get_xaxis().set_ticks([])
            ax[2,2].get_yaxis().set_ticks([])
            
            ax[2,3].plot(xrange, theta[0,2]/I_sum*vti.intensity(xrange, phi_minusy), color = 'k')
            ax[2,3].plot(theta[0,1], theta[0,2]/I_sum*vti.intensity(theta[0,1], phi_minusy), marker='*', mec='k', mfc='w', ms = 12)
