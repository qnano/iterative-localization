...
2. Verify that the code and the Datasets.xlsx file are contained in the directory "/iterative_localization/".
...
9. Run the appropriate simulation program(s):
...
1. Verify that the directory "iterative_localization/SimData/" exists and that it contains the necessary and desired simulation data. Additionally, verify that the code and the Datasets.xlsx file are contained in the directory "/iterative_localization/". If this is not the case, please refer back to the previous sections.

2. To produce the desired figures from the article, open the appropriate plotting program(s):
...
3. Run the appropriate program(s) to process and plot the data. By default, this will produce all individual figure panels and legends.

4. To display specific figures or panels, you can use the function ```plot(plotnumber, panel)``` (in plot_precision.py and FT_plot_precision.py) or ```plot(plotnumber)``` (in plot_samples.py and FT_plot_samples.py) after running the program once. To do so:
...
In this program, the parameters c_k are chosen such that the imaging 
